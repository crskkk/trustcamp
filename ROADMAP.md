...
## Merged
- `v1.4` — 0104 NPC camo + locomotion (motion policy `chooseInput` (pure, deterministic, ≤ player speed), per-tick `bridge.sendState` loop, Unity `NpcController` + `NpcSpawner` reusing the same `PlayerController.Step()` path with no `isNpc` branch, edit-mode locomotion tests, e2e that asserts the publish loop ticks the chip position every ~2 s). Shell→Unity live glue for the broadcast path still lands in 0105.
