...
// Task 0103 e2e: dev-only. The Bootstrap component seeds 3 deterministic
...
test("dev spawns 3 Prospect NPCs and Clear NPCs empties the chip strip", async ({ page }) => {
  ...
  await expect(npcChips).toHaveCount(3);
  ...
  await expect(page.getByTestId("npc-count")).toHaveText("3");
